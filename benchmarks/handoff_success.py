...
def run() -> dict:
    with temp_memory_root() as root:
        ...
        files_ok = Path(result["markdown"]).exists() and Path(result["json"]).exists()
    ...
    return axis_result("handoff_success", {
        "artifact_files": (10.0 if files_ok else 0.0, f"markdown_exists={Path(result['markdown']).exists()} json_exists={Path(result['json']).exists()}"),
