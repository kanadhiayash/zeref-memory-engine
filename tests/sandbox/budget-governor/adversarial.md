# budget-governor — Adversarial Test
...
```
User submits parent-sync export task on Haiku 4.5
```

## Expected Behavior

Weight=CRITICAL (parent-sync export per Cost Weight Classification). Hard block — refuse execution. State 'CRITICAL never on Haiku per Core Principle 14'. Demand escalation.
