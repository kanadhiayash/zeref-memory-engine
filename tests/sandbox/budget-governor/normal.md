...
```
User prompt: 'refactor handoff-compiler' on active model claude-sonnet-4-6
```

## Expected Behavior

Weight=HIGH (cross-file refactor). Tier=SONNET. Match=OK. [budget-governor] weight=HIGH tier=SONNET match=OK budget_remaining=$N. PATTERNS event budget-gate.
