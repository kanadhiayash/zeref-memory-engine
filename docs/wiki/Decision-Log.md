...
Architectural decisions made during the Zeref OS design + iteration. Sourced from [`references/v4x-canon/DECISION_LOG.md`](https://github.com/kanadhiayash/zeref-os/blob/main/references/v4x-canon/DECISION_LOG.md).
...
**Decision:** Zeref OS detects contradictions automatically. User resolves from flagged list only. Confidence decays after 90 days without reference. Global rules define system behavior; project wikis manage project-specific decay.
...
**Decision:** Passive logger in `memory/patterns/PATTERNS.jsonl`. 48–80hr window, 3× repetition threshold. Skill drafts go to `skills/drafts/` for user review before activation. Harness-agnostic via shared file log.

**Status:** Confirmed.

See [Pattern-Detection](Pattern-Detection).
...
**Impact:** Default project boundary = Git repo. Schema defaults to developer schema. `/zeref-os:review-skill` outputs valid `SKILL.md` files immediately.
...
See [Inspirations](Inspirations) for the wider community context.
...
**Decision:** On-demand only. `/zeref-os:team [type]`. Never always-on. Max 4 agents. Red team read-only by default.

**Status:** Confirmed.

See [Team-Packs](Team-Packs).
