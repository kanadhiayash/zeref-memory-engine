...
| `v1.1.0` | 2026-07-10 | Audit remediation release — closes the Repository-Wide Consistency Audit (records moved to private `zeref-operator-records` repo). New `SOUL.md`, `zeref/security/policy.py` (runtime enforcement of PRIVACY/SHARING_POLICY/PERMISSIONS), Registry v1.1 (agents+commands+team_packs+gates arrays), fix `pyproject.toml` build-backend, fix `ci.yml` YAML, evidence-state HARNESS_MATRIX, SHA-bound release evidence. | `ZEREF_CONSISTENCY_AUDIT.md` (private repo) — 40 findings, 15 P0 closed |
